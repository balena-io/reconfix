...
exports.readConfiguration = (schema, image) => {
  return filesystem.readImageConfiguration(schema, image)
    .then(_.partial(configuration.extract, schema));
};
...
exports.writeConfiguration = (schema, object, image) => {
  return filesystem.readImageConfiguration(schema, image).then((data) => {

    _.assignWith(data, configuration.generate(schema, object), (objectValue, sourceValue) => {
    ...
    return filesystem.writeImageConfiguration(schema, image, data);
  });
};
