...
exports.readImageData = (schema, image) => {
  ...
  return Bluebird.map(_.toPairs(realFiles), (filePair) => {
    ...
    if (fileDeclaration.fileset) {
      return imagefs.interact(
        ...
        (fs) => {
          ...
          return readdirAsync(fileDeclaration.location.path);
        }
      )
        .then((files) => {
          return Bluebird.map(files, (file) => {
            return imagefs.interact(
              ...
              (fs) => {
                ...
                return readFileAsync(path.join(fileDeclaration.location.path, file)).then((b) => {
                  return b.toString();
                });
              }
            ).then((contents) => {
    ...
    return imagefs.interact(
      ...
      (fs) => {
        const readFileAsync = Bluebird.promisify(fs.readFile);
        return readFileAsync(fileDeclaration.location.path).then((b) => {
          return b.toString();
        });
      }
    )
...
exports.writeImageData = (schema, manifest, image) => {
  return Bluebird.each(_.toPairs(manifest), (filePair) => {
      ...
      return Bluebird.each(_.toPairs(fileDeclaration.data), (childFilePair) => {
        ...
        return imagefs.interact(
          ...
          (fs) => {
            ...
            return writeFileAsync(path.join(fileDeclaration.location.path, childFileName), childFileContents);
          }
        );
      });
    }
    ...
    return imagefs.interact(
      ...
      (fs) => {
        const writeFileAsync = Bluebird.promisify(fs.writeFile);
        return writeFileAsync(fileDeclaration.location.path, fileDeclaration.data);
      }
    );
  });
};
