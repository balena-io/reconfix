...
exports.getPartitionNumber = (partition) => {
  return partition.primary + (partition.logical || 0);
};

exports.mountPartition = (disk, options) => {
  // Mounts a partition and umounts it when you're done using it.
  return lkl.mountAsync(disk, options)
  .then((mountpoint) => {
    return [ options.partition, mountpoint ];
  })
  .disposer((partitionAndMountpoint) => {
    return lkl.umountAsync(partitionAndMountpoint[1]);
  });
};

const mountAllPartitions = (schema, image, readonly) => {
  readonly = readonly === true;
  const realFiles = _.omitBy(schema, exports.isSchemaFileVirtual);

  // TODO: update lkl & add readOnly
  const disk = new lkl.disk.FileDisk(image);
  const partitionNumbers = _.map(_.values(realFiles), (dict) => {
    return exports.getPartitionNumber(dict.location.partition);
  });
  return _.map(partitionNumbers, (number) => {
    const options = {
      readOnly: readonly,
      filesystem: 'vfat',
      partition: number
    };
    return exports.mountPartition(disk, options);
  });
};
...
exports.readImageData = (schema, image) => {
  return Bluebird.using(mountAllPartitions(schema, image, true), function(mpoints) {
    const mountpoints = _.fromPairs(mpoints);
    const realFiles = _.omitBy(schema, exports.isSchemaFileVirtual);
    return Bluebird.map(_.toPairs(realFiles), (filePair) => {
      const fileId = _.first(filePair);
      const fileDeclaration = _.last(filePair);
      const number = exports.getPartitionNumber(fileDeclaration.location.partition);
      const fpath = path.join(mountpoints[number], fileDeclaration.location.path);
      if (fileDeclaration.fileset) {
        return lkl.fs.readdirAsync(fpath)
        .then((files) => {
          files = files.filter((name) => {
            return !name.startsWith('.');
          });
          return Bluebird.map(files, (file) => {
            return lkl.fs.readFileAsync(path.join(fpath, file), 'utf8')
            .then((contents) => {
...
exports.writeImageData = (schema, manifest, image) => {
  return Bluebird.using(mountAllPartitions(schema, image), function(mpoints) {
    const mountpoints = _.fromPairs(mpoints);
    return Bluebird.each(_.toPairs(manifest), (filePair) => {
      ...
      if (_.get(schema, fileId).fileset) {
        return Bluebird.each(_.toPairs(fileDeclaration.data), (childFilePair) => {
          ...
          return lkl.fs.writeFileAsync(path.join(fpath, childFileName), childFileContents);
        });
      }

      return lkl.fs.writeFileAsync(fpath, fileDeclaration.data);
    });
  });
};
