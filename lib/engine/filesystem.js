...
exports.readImageData = (schema, image) => {
  ...
  return Bluebird.map(_.toPairs(realFiles), ([ fileId, fileDeclaration ]) => {
    return imagefs.read({
      image: image,
    ...
    .then(rindle.extract)
    .then(_.partial(formats.parse, fileDeclaration.type))
    .then((data) => {
      return [
        fileId,
        {
...
exports.readImageConfiguration = (schema, image) => {
  return exports.readImageData(schema, image).then((imageFileDeclarations) => {
    const imageData = _.mapValues(imageFileDeclarations, (fileDeclaration) => {
      return fileDeclaration.data;
    });

    const virtualFiles = _.pickBy(schema, exports.isSchemaFileVirtual);
    _.each(virtualFiles, (fileDeclaration, fileId) => {
      const fileContentsProperty = _.concat([
        fileDeclaration.location.parent
      ], fileDeclaration.location.property);

      const fileContents = _.get(imageData, fileContentsProperty);
      _.set(imageData, fileId, formats.parse(fileDeclaration.type, fileContents));
      _.unset(imageData, fileContentsProperty);
    });
