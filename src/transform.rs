...
pub fn transform_wet(config: Vec<Entry>, schema: &Schema) -> Result<Value> {
    ...
    let mut mapped = config.into_iter()
    ...
    for (name, file) in ordered {
        let wet_content = match &file.location {
            ...
            &Location::Dependent { ref parent, ref location } => {
...
mod tests {
    ...
    fn wet_to_dry_dependent_json() {
        ...
        let schema = Schema { files: files };
        let entry = Entry { name: "independent".into(), content: wet_content };
