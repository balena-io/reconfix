...
exports.splitTemplateTokens = (template) => {
  const result = _.reduce(template, (accumulator, character) => {
    if (accumulator.isExpression) {
      accumulator.buffer += character;

      if (character === ']') {
        accumulator.words.push(accumulator.buffer);
        accumulator.buffer = '';
  ...
  if (result.buffer) {
    result.words.push(result.buffer);
  }
