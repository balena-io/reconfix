...
// TODO: Move to `jsontemplate`
const choiceMatches = (domain, template) => {
  const data = jsontemplate.decompile(template, domain);
  const result = jsontemplate.compile(template, data);
  return _.isEqual(result, domain);
};
    ...
    _.each(domainFilenamesPaths, (domainFilenamePath) => {
      ...
      if (correspondence.choice) {
        const matches = _.filter(correspondence.choice, (choice) => {
          return choiceMatches(domain, choice.template);
        });
